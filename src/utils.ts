...
/**
 * Define tipos para la estructura de tema de iconos
 */
interface IconThemeDefinition {
  iconDefinitions: Record<string, { iconPath: string }>;
  folders: Array<{ icon: string; name: string }>;
  foldersExpanded: Array<{ icon: string; name: string }>;
  ...
  light: {
    folder: { iconPath: string };
    folderExpanded: { iconPath: string };
    folders: Array<{ icon: string; name: string }>;
    foldersExpanded: Array<{ icon: string; name: string }>;
  };
  highContrast: {
  ...
  // Propiedades clave para asignar iconos por defecto
  folder: string;
 ...
 */
export function generateFileIconsStructure(baseDir: string): void {
  ...
  console.log(
    `Estructura de archivos para temas de iconos creada en: ${fileiconsDir}`
  );
  console.log(
    "Para completar la instalación, coloca los archivos SVG en las respectivas carpetas de cada framework."
  );
}
 ...
 */
function generateThemeJson(framework: string): IconThemeDefinition {
  ...
  const themeJson: IconThemeDefinition = {
    iconDefinitions: {
  ...
  for (const folderName of commonFolders) {
    ...
    if (framework !== "default") {
      ...
      const frameworkFolderPath = path.join(
        __dirname, 
        "..", 
        "fileicons", 
        framework, 
        `folder_${folderName}.svg`
      );
 ...
 */
function getFrameworkSpecificFolders(framework: string): string[] {
  switch (framework) {
    case "react":
      return [
        "components",
 ...
 */
export function generateCommonSvgs(baseDir: string): void {
  ...
  for (const [framework, color] of Object.entries(frameworkBaseColors)) {
    ...
    fs.writeFileSync(
      path.join(commonDir, `folder-${framework}.svg`),
      createFolderSvg(color)
    );

    // Icono de carpeta abierta
    fs.writeFileSync(
      path.join(commonDir, `folder-expanded-${framework}.svg`),
      createFolderOpenSvg(color)
    );

    // Icono de archivo
    fs.writeFileSync(
      path.join(commonDir, `file-${framework}.svg`),
      createFileSvg(color)
    );
  }
  ...
  const commonColors: Record<string, string> = {
    src: "#42A5F5", // azul claro
    public: "#66BB6A", // verde claro
    test: "#EF5350", // rojo claro
    docs: "#7E57C2", // morado claro
    dist: "#78909C", // gris azulado
  ...
  for (const folderName of commonFolders) {
    ...
    // Icono de carpeta cerrada
    fs.writeFileSync(
      path.join(commonDir, `folder-${folderName}.svg`),
      createFolderSvg(color)
    );

    // Icono de carpeta abierta
    fs.writeFileSync(
      path.join(commonDir, `folder-${folderName}-expanded.svg`),
      createFolderOpenSvg(color)
    );
  }
 ...
 */
export function getFolderColor(framework: string, folderName: string): string {
  ...
  const frameworkColors: Record<string, Record<string, string>> = {
    react: {
 ...
 */
function createFolderSvg(color: string): string {
 ...
 */
function createFolderOpenSvg(color: string): string {
  return `<?xml version="1.0" encoding="UTF-8"?>
<svg width="24px" height="24px" viewBox="0 0 24 24" version="1.1" xmlns="http://www.w3.org/2000/svg">
  <g stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
 ...
 */
function createFileSvg(color: string): string {
  return `<?xml version="1.0" encoding="UTF-8"?>
<svg width="24px" height="24px" viewBox="0 0 24 24" version="1.1" xmlns="http://www.w3.org/2000/svg">
  <g stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
